/*
1. Выводить счетчик взорванных бомб.
2. Убрать границы поля для передвижения персонажа. 
Т.е. при пересечении границы поля бомбермен появляется с противоположной стороны.
*/
import {
    tickPerSecond,
    gameFieldWidth,
...
const bombCountInField = document.getElementById("bombCount");
let gameField;
const gameState = {
    bomberman: {
        position: {
...
const setBombermanPosition = (rowNumber, columnNumber) => {
    ...
    if (currentBombermanPositionElement !== null) {
        const bombermanPosition = getPositionById(
            currentBombermanPositionElement
        );
    ...
    document.body.addEventListener("keydown", (event) => {
        switch (event.key) {
            ...
            case " ":
                // поставить бомбу
                gameState.bombCount++;
                bombCountInField.innerHTML = gameState.bombCount;
                setBombPosition(
                    gameState.bomberman.position.rowNumber,
                    gameState.bomberman.position.columnNumber
                );
...
const checkBombsStop = () => {
    ...
    for (const explodeCellKey in gameState.bombExploseCells) {
        if (
            ...
            // убрать из DOM класс
            const [explodeCellRow, explodeCellColumn] =
                explodeCellKey.split(",");
            const explodeCell = getCellDomElement(
                explodeCellRow,
                explodeCellColumn
            );
